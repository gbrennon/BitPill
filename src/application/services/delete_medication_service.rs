...
use crate::application::ports::inbound::delete_medication_port::{DeleteMedicationPort, DeleteMedicationRequest, DeleteMedicationResponse};
...
impl DeleteMedicationPort for DeleteMedicationService {
    fn execute(&self, request: DeleteMedicationRequest) -> Result<DeleteMedicationResponse, ApplicationError> {
        let uuid = Uuid::parse_str(&request.id).map_err(|_| ApplicationError::InvalidInput("invalid id".into()))?;
