...
pub fn schedule_time<'a>(count: usize, values: &'a [String], selected_idx: usize, focused: bool) -> List<'a> {
    let items: Vec<ListItem> = (0..count)
        .map(|i| {
            let raw = values.get(i).map(|s| s.as_str()).unwrap_or("");
            let display = if raw.trim().is_empty() { "HH:MM:SS" } else { raw };
            let prefix = if focused && i == selected_idx { "> " } else { "  " };
            // All schedule items use BORDER_COLOR; do not bold to keep only title bold
            let item_style = content_style().fg(BORDER_COLOR);
            ListItem::new(Line::from(Span::styled(format!("{}{}", prefix, display), item_style)))
        })
        .collect();

    let title_style = if focused {
        content_style().fg(BORDER_COLOR).add_modifier(Modifier::BOLD)
    } else {
