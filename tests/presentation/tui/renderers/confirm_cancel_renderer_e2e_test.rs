use bitpill::{
    presentation::tui::{
        ...
        draw,
        renderers::{confirm_cancel_renderer::ConfirmCancelRenderer, ScreenRenderer},
        screen::Screen,
    },
...
mod tests {
    ...
    #[test]
    fn confirm_cancel_renderer_e2e_renders_modal() {
    ...
    terminal.draw(|f| {
        let screen = Screen::ConfirmCancel { previous: Box::new(Screen::HomeScreen) };
    ...
    let contains_discard = buffer.content.iter().any(|cell| 
        cell.symbol().contains("Discard") || cell.symbol().contains("changes")
    );
    assert!(contains_discard, "Expected discard changes modal to be rendered");
    }

    #[test]
    fn confirm_cancel_renderer_e2e_with_previous_screen() {
    let dir = tempdir().unwrap();
    std::fs::write(dir.path().join("medications.json"), r#"[{"id":"m1","name":"Test"}]"#).unwrap();
    std::fs::write(dir.path().join("doses.json"), "[]").unwrap();
    std::fs::write(dir.path().join("settings.json"), r#"{"vim_enabled":false}"#).unwrap();
