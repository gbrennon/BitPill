...
fn execute_marks_existing_dose_record_as_taken() {
    ...
    let service = MarkDoseTakenService::new(Arc::new(FakeDoseRecordRepository::with(record)), med_repo);
...
fn execute_when_find_by_id_fails_returns_storage_error() {
    use bitpill::application::errors::ApplicationError;
    let med_repo = Arc::new(crate::fakes::FakeMedicationRepository::new());
    let service = MarkDoseTakenService::new(Arc::new(FakeDoseRecordRepository::failing_on_find_by_id()), med_repo);
...
fn execute_when_save_fails_after_mark_taken_returns_storage_error() {
    ...
    let record_id = record.id().clone();
    let med_repo = Arc::new(crate::fakes::FakeMedicationRepository::new());
    let service = MarkDoseTakenService::new(Arc::new(FakeDoseRecordRepository::with_failing(record)), med_repo);
